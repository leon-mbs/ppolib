## Pure  PHP библиотека  для наложения цифровой подписи  (КЕП)  согласно ДСТУ-4145

Разрабатывалась для [Zippy CRM](https://github.com/leon-mbs/zstore) но может использоватся  с любым PHP приложением, 
в том числе  и на виртуальном хостинге  где нет  возможности ставить  стороннее ПО  

Большинство  кода  портировано с <https://github.com/dstucrypt/jkurwa> часть с <https://github.com/storojs72/bc-dstu-csharp>   
...
### Как  использовать
   ...
   Распаковка сертификата и соответствующего  ему ключа (сертификат может  быть для  подписи или  шифрования и  соответственно разные  ключи)     
   ```
   ...
   :warning:  Поскольку  распаковка ключа с  контейнера   происходит  довольно  медленно, обьекты     $cert и $key  следует 
